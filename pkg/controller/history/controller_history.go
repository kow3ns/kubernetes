...
// ControllerRevisionName returns the Name for a ControllerRevision in the form prefix-hash-revision. If the length
...
// parent is of parentType. The ControllerRevision has labels matching selector, contains Data equal to data, and
...
type Interface interface {
	...
	// Object of parentType is the owner of revision. If revision is already owned, an error is returned. If the
	// resource patch fails, an error is returned. If no error is returned, the returned ControllerRevision is
	// valid.
	AdoptControllerRevision(parent metav1.Object, parentType *metav1.TypeMeta, revision *apps.ControllerRevision) (*apps.ControllerRevision, error)
...
func (rh *realHistory) UpdateControllerRevision(revision *apps.ControllerRevision, newRevision int64) (*apps.ControllerRevision, error) {
	...
	err = retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		_, updateErr := rh.client.Apps().ControllerRevisions(clone.Namespace).Update(clone)
		if updateErr == nil {
			return nil
		}
...
func (rh *realHistory) AdoptControllerRevision(parent metav1.Object, parentType *metav1.TypeMeta, revision *apps.ControllerRevision) (*apps.ControllerRevision, error) {
	...
	return rh.client.Apps().ControllerRevisions(parent.GetNamespace()).Patch(revision.GetName(),
		types.StrategicMergePatchType, []byte(fmt.Sprintf(
			...
			parent.GetResourceVersion(), parentType.Kind,
...
func (fh *fakeHistory) AdoptControllerRevision(parent metav1.Object, parentType *metav1.TypeMeta, revision *apps.ControllerRevision) (*apps.ControllerRevision, error) {
	blockOwnerDeletion := true
	isController := true
	if owner := controller.GetControllerOf(revision); owner != nil {
		return nil, fmt.Errorf("attempt to adopt revision owned by %v", owner)
