...
func TestRealHistory_ListControllerRevisions(t *testing.T) {
	...
	ss1Rev1, err := NewControllerRevision(&ss1.ObjectMeta, &ss1.TypeMeta, sel1, rawTemplate(&ss1.Spec.Template), 1)
	if err != nil {
		t.Fatal(err)
	}
	ss1Rev1.Namespace = ss1.Namespace
	ss1Rev2, err := NewControllerRevision(&ss1.ObjectMeta, &ss1.TypeMeta, sel1, rawTemplate(&ss1.Spec.Template), 2)
	if err != nil {
		t.Fatal(err)
	}
	ss1Rev2.Namespace = ss1.Namespace
	ss2Rev1, err := NewControllerRevision(&ss2.ObjectMeta, &ss2.TypeMeta, sel2, rawTemplate(&ss2.Spec.Template), 1)
	if err != nil {
		t.Fatal(err)
	}
	ss2Rev1.Namespace = ss2.Namespace
	ss1Orphan, err := NewControllerRevision(&ss1.ObjectMeta, &ss1.TypeMeta, sel1, rawTemplate(&ss1.Spec.Template), 3)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRealHistory_DeleteControllerRevision(t *testing.T) {
	...
	ss2Rev1.Namespace = ss2.Namespace
	ss2Rev2, err := NewControllerRevision(&ss2.ObjectMeta, &ss2.TypeMeta, sel2, rawTemplate(&ss2.Spec.Template), 2)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRealHistory_AdoptControllerRevision(t *testing.T) {
	...
	testFn := func(test *testcase, t *testing.T) {
		...
		adopted, err := history.AdoptControllerRevision(test.parent, test.parentType, test.revision)
		if !test.err && err != nil {
			t.Errorf("%s: %s", test.name, err)
		}
...
func TestSortControllerRevisions(t *testing.T) {
	...
	ss1Rev2.Namespace = ss1.Namespace
	ss1Rev3, err := NewControllerRevision(&ss1.ObjectMeta, &ss1.TypeMeta, sel1, rawTemplate(&ss1.Spec.Template), 2)
	if err != nil {
		t.Fatal(err)
	}
