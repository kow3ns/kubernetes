...
type StatefulSetControlInterface interface {
	...
	// error is not nil, the returns slice of ControllerRevisions is valid.
...
func (ssc *defaultStatefulSetControl) UpdateStatefulSet(set *apps.StatefulSet, pods []*v1.Pod) error {
	...
	currentRevision, updateRevision, err := ssc.getSetRevisions(set, revisions)
...
func (ssc *defaultStatefulSetControl) AdoptOrphanRevisions(
	...
	for i := range revisions {
		adopted, err := ssc.controllerHistory.AdoptControllerRevision(set, &set.TypeMeta, revisions[i])
		if err != nil {
			return err
		}
		revisions[i] = adopted
...
// CurrentRevision in set's Status are considered to be live, as are any revisions associated with the Pods in pods.
// Non-live revisions are deleted, starting with the revision with the lowest Revision, until only RevisionHistoryLimit
// revisions remain. If the returned error is nil the operation was successful.
func (ssc *defaultStatefulSetControl) truncateHistory(
	...
	if len(history) < int(*set.Spec.RevisionHistoryLimit) {
		return nil
	}
	// delete any non-live history to maintain the revision limit.
	history = history[:int(*set.Spec.RevisionHistoryLimit)]
...
// getSetRevisions returns the current and update ControllerRevisions for set. This method may create a new revision,
// or modify the Revision of an existing revision if an update to set is detected.
func (ssc *defaultStatefulSetControl) getSetRevisions(
	set *apps.StatefulSet,
...
func (ssc *defaultStatefulSetControl) updateStatefulSet(set *apps.StatefulSet,
	...
	// get the current and update revisions of the set. Note that computed
	...
	*status.ObservedGeneration = set.Generation
	...
	for i := range replicas {
		...
		if isTerminating(replicas[i]) && monotonic {
			glog.V(2).Infof(
				"StatefulSet %s is waiting for Pod %s to Terminate",
				set.Name,
				replicas[i].Name)
			return &status, nil
		}
		...
		if !isRunningAndReady(replicas[i]) && monotonic {
			glog.V(2).Infof(
				"StatefulSet %s is waiting for Pod %s to be Running and Ready",
				set.Name,
				replicas[i].Name)
			return &status, nil
		}
	...
	for target := len(condemned) - 1; target >= 0; target-- {
		...
		if getPodRevision(pods[target]) == currentRevision.Name {
			status.CurrentReplicas--
		} else if getPodRevision(pods[target]) == updateRevision.Name {
			status.UpdatedReplicas--
		}
		if monotonic {
			return &status, nil
	...
	for target := len(replicas) - 1; target >= updateMin; target-- {
		if getPodRevision(replicas[target]) != updateRevision.Name {
			glog.V(2).Infof("StatefulSet %s terminating Pod %s for update", set.Name, replicas[target])
